...
def get_bert(bert_model, bert_do_lower_case):
    # Avoid a hard dependency on BERT by only importing it if it's being used
    from pretrained_bert import BertModel, BertTokenizer
...
class BiLinear(nn.Module):
    ...
    def forward(self, input_left, input_right):
        ...
        output = (
            output
            + nn.functional.linear(input_left, self.W_l, None)
            + nn.functional.linear(input_right, self.W_r, None)
        )
...
class LabelAttention(nn.Module):
    ...
    def __init__(
        ...
        self.combine_as_self = (
            hparams.lal_combine_as_self
        ...
        if self.partitioned:
            ...
            if self.q_as_matrix:
                self.w_qs1 = nn.Parameter(
                    torch_t.FloatTensor(self.d_l, self.d_content, d_k // 2),
                    requires_grad=True,
                )
            else:
                self.w_qs1 = nn.Parameter(
                    torch_t.FloatTensor(self.d_l, d_k // 2), requires_grad=True
                )
            self.w_ks1 = nn.Parameter(
                torch_t.FloatTensor(self.d_l, self.d_content, d_k // 2),
                requires_grad=True,
            ...
            self.w_vs1 = nn.Parameter(
                torch_t.FloatTensor(self.d_l, self.d_content, d_v // 2),
                requires_grad=True,
            )

            if self.q_as_matrix:
                self.w_qs2 = nn.Parameter(
                    torch_t.FloatTensor(self.d_l, self.d_positional, d_k // 2),
                    requires_grad=True,
                )
            else:
                self.w_qs2 = nn.Parameter(
            ...
            self.w_vs2 = nn.Parameter(
                torch_t.FloatTensor(self.d_l, self.d_positional, d_v // 2),
                requires_grad=True,
            )
            ...
            if self.q_as_matrix:
                self.w_qs = nn.Parameter(
                    torch_t.FloatTensor(self.d_l, d_model, d_k), requires_grad=True
                )
            else:
                self.w_qs = nn.Parameter(
                    torch_t.FloatTensor(self.d_l, d_k), requires_grad=True
                )
            self.w_ks = nn.Parameter(
                torch_t.FloatTensor(self.d_l, d_model, d_k), requires_grad=True
            )
            self.w_vs = nn.Parameter(
                torch_t.FloatTensor(self.d_l, d_model, d_v), requires_grad=True
            )
    ...
    def combine_v(self, outputs):
        ...
        if not self.partitioned:
            ...
            if self.combine_as_self:
                outputs = (
                    torch.transpose(outputs, 0, 1).contiguous().view(-1, d_l * self.d_v)
                )
            else:
            ...
            if self.combine_as_self:
                outputs1 = (
                    torch.transpose(outputs1, 0, 1).contiguous().view(-1, d_l * d_v1)
                )
                outputs2 = (
                    torch.transpose(outputs2, 0, 1).contiguous().view(-1, d_l * d_v1)
                )
            else:
    ...
    def forward(self, inp, batch_idxs, k_inp=None):
        ...
        if self.use_resdrop:
            if self.combine_as_self:
                ...
                outputs = torch.cat(
                    [
                        self.residual_dropout(outputs[:, i, :], batch_idxs).unsqueeze(1)
...
class ChartParser(nn.Module):
    def __init__(
        ...
        if hparams.use_tupe:
            self.tupe_fastBPE = fastBPE.fastBPE(
                pkg_resources.resource_filename(
                    fairseq.__name__, "../tupe_ckp/bpe-code"
                )
            )
            self.tupe_args = torch.load(
                pkg_resources.resource_filename(
                    fairseq.__name__, "../tupe_ckp/model.pt"
                )
            )
            from fairseq import checkpoint_utils

            self.tupe = checkpoint_utils.load_model_ensemble(
                [
                    pkg_resources.resource_filename(
                        fairseq.__name__, "../tupe_ckp/model.pt"
                    )
            ...
            with open(
                pkg_resources.resource_filename(
                    fairseq.__name__, "../tupe_ckp/dict.txt"
                )
            ) as dict_f:
        ...
        if hparams.use_roberta:
            ...
            self.roberta_max_len = (
                self.roberta.embeddings.position_embeddings.num_embeddings
            )
        ...
        annotation_dim = (
            ((label_vocab.size - 2) * self.lal_d_proj)
            if (self.use_lal and not self.lal_combine_as_self)
            else hparams.d_model
        )
    ...
    def from_spec(cls, spec, model):
        ...
        for (
            param
        ) in "use_lal lal_resdrop lal_pwff lal_partitioned lal_q_as_matrix lal_combine_as_self".split(
            " "
        ):
    ...
    def parse_batch(self, sentences, golds=None):
        ...
        for snum, sentence in enumerate(sentences):
            ...
            for (tag, word) in [(START, START)] + sentence + [(STOP, STOP)]:
                tag_idxs[i] = (
                    0
                    if (not self.use_tags and self.f_tag is None)
        ...
        if self.tupe is not None:
            all_input_ids = np.zeros((len(sentences), self.tupe_max_len), dtype=int)
            all_input_mask = np.zeros((len(sentences), self.tupe_max_len), dtype=int)
            all_word_start_mask = np.zeros(
            ...
            for snum, sentence in enumerate(sentences):
                ...
                # word = self.tupe_dict.get(word, self.tupe_dict["unk"])
                # if word == "n't" and cleaned_words:
                #     cleaned_words[-1] = cleaned_words[-1] + "n"
                #     word = "'t"
                # cleaned_words.append(word)
                ...
                for i, word in enumerate(cleaned_words):
                    if i == 0 or i == len(cleaned_words) - 1:
                        word_tokens = ["<s>"]
                    elif i == len(cleaned_words) - 1:
                        word_tokens = ["</s>"]
                    else:
                ...
                # Não precisamos mais, pois já temos os tokens de início e fim de sentença
                # tokens.append("[SEP]")
                # word_start_mask.append(1)
                # word_end_mask.append(1)
        ...
        if self.encoder is not None:
            ...
            if self.use_lal and not self.lal_combine_as_self:
                ...
                fencepost_annotations = torch.cat(
                    [
                        annotations[
                            :-1,
                            (i * self.lal_d_proj) : (i * self.lal_d_proj + half_dim),
                        ]
                        for i in range(d_l)
                    ]
                    + [
            ...
            fencepost_annotations_start = features.masked_select(
                all_word_start_mask.to(DTYPE).unsqueeze(-1)
            ).reshape(-1, features.shape[-1])
        ...
        for snum, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
            ...
            type_gather = [
                self.type_vocab.index(leaf.type) for leaf in golds[snum].leaves()
            ]
            cun += 1
            ...
            loss = (
                ...
                * self.loss_func(
                    arc_score[1:, :],
                    from_numpy(np.array(arc_gather)).requires_grad_(False),
                )
                + 0.5
                * self.loss_funt(
                    type_score[1:, :],
                    from_numpy(np.array(type_gather)).requires_grad_(False),
                )
            )
    ...
    def label_scores_from_annotations(
        self, fencepost_annotations_start, fencepost_annotations_end
    ):
        ...
        if self.contributions and self.use_lal:
            contributions = np.zeros(
                (
                    span_features.shape[0],
                    span_features.shape[1],
                    span_features.shape[2] // self.lal_d_proj,
                )
            )
            ...
            for i in range(contributions.shape[0]):
                for j in range(contributions.shape[1]):
                    for l in range(contributions.shape[-1]):
                        contributions[i, j, l] = (
                            span_features[i, j, l * half_dim : (l + 1) * half_dim].sum()
                            + span_features[
                                i,
                                j,
                                half_vector
                                + l * half_dim : half_vector
                                + (l + 1) * half_dim,
                            ].sum()
                        )
                    contributions[i, j, :] = contributions[i, j, :] - np.min(
                        contributions[i, j, :]
                    )
