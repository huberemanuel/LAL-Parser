...
def run_train(args, hparams):
    ...
    while dev_inst is not None:
        ...
        dep_dev_data.append(
            (sent.words, dev_inst.postags, dev_inst.heads, dev_inst.types)
        )
    ...
    def check_dev(epoch_num):
        ...
        dev_pred_head = [
            [leaf.father for leaf in tree.leaves()] for tree in dev_predicted
        ]
        dev_pred_type = [
            [leaf.type for leaf in tree.leaves()] for tree in dev_predicted
        ]
        assert len(dev_pred_head) == len(dev_pred_type)
        assert len(dev_pred_type) == len(dep_dev_type)
        stats, stats_nopunc, stats_root, num_inst = dep_eval.eval(
            len(dev_pred_head),
            dep_dev_word,
            dep_dev_pos,
            dev_pred_head,
        ...
        print(
            ...
            % (
                ...
                dev_lcorr,
                dev_total,
                dev_ucorr * 100 / dev_total,
                dev_lcorr * 100 / dev_total,
                dev_ucomlpete * 100 / dev_total_inst,
                dev_lcomplete * 100 / dev_total_inst,
            )
        )
        print(
            ...
            % (
                ...
                dev_ucomlpete_nopunc * 100 / dev_total_inst,
...
def run_test(args):
    ...
    test_pred_head = [
        [leaf.father for leaf in tree.leaves()] for tree in test_predicted
    ]
    ...
    stats, stats_nopunc, stats_root, test_total_inst = dep_eval.eval(
        len(test_pred_head),
        dep_test_word,
        dep_test_pos,
        test_pred_head,
    ...
    (
        ...
        test_lcorrect,
        test_total,
        test_ucomlpete_match,
        test_lcomplete_match,
    ) = stats
    (
        test_ucorrect_nopunc,
        test_lcorrect_nopunc,
        test_total_nopunc,
        test_ucomlpete_match_nopunc,
        test_lcomplete_match_nopunc,
    ) = stats_nopunc
    ...
    print(
        ...
        % (
            ...
            test_lcomplete_match * 100 / test_total_inst,
        )
    )
    print(
    ...
    print(
        "best test Root: corr: %d, total: %d, acc: %.2f%%"
        % (
            test_root_correct,
            test_total_root,
...
def run_parse(args):
    ...
    if args.max_tokens > 0:
        ...
        for sentence in sentences:
            sub_sentences = [
                word_tokenize(sub_sentence) for sub_sentence in sent_tokenize(sentence)
            ]
            this_sentence = sub_sentences[0][: args.max_tokens]
            this_idx = 1
            move_on = False
            while (
                this_idx < len(sub_sentences)
                and len(this_sentence) < args.max_tokens
                and not move_on
            ):
    ...
    for start_index in tqdm(
        range(0, len(sentences), args.eval_batch_size), desc="Parsing sentences"
    ):
        subbatch_sentences = sentences[start_index : start_index + args.eval_batch_size]
        if args.pos_tag == 2:
            tagged_sentences = [
                [
                    (dummy_tag, REVERSE_TOKEN_MAPPING.get(word, word))
            ...
            tagged_sentences = [
                [
                    (
                        REVERSE_TOKEN_MAPPING.get(tag, tag),
                        REVERSE_TOKEN_MAPPING.get(word, word),
                    )
            ...
            tagged_sentences = [
                [
                    (
                        REVERSE_TOKEN_MAPPING.get(
                            word.split("_")[0], word.split("_")[0]
                        ),
                        REVERSE_TOKEN_MAPPING.get(
                            word.split("_")[1], word.split("_")[1]
                        ),
                    )
...
def main():
    ...
    subparser.add_argument(
        "--dep-train-ptb-path", default="data/ptb_train_3.3.0.sd.clean"
    )
    ...
    subparser.add_argument("--model-path-base", required=True)
    subparser.add_argument(
        "--contributions", type=int, default=1
    )  # 1 to print contributions
    subparser.add_argument(
        "--pos-tag", type=int, default=1
